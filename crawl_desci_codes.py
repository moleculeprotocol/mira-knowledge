...
async def main():
    domain_filter = DomainFilter(allowed_domains=["desci-codes.gitbook.io"])
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        results = await crawler.arun(
            url="https://desci-codes.gitbook.io/desci.codes", config=config
        )
        ...
        # Save to JSON file
        with open("data/desci_codes.json", "w", encoding="utf-8") as f:
            json.dump(pages_data, f, indent=2, ensure_ascii=False)

        print(f"\n💾 Saved {len(pages_data)} pages to data/desci_codes.json")
        print(
